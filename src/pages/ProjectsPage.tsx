...
function CreateProjectForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) return;
    await createProject(name.trim(), mode);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
        ...
        <select
          ...
          className="w-full border border-input rounded-lg px-3 py-2 text-[16px] mt-1 mb-6 bg-white"
        >
          {meetingModes.map((m) => (
...
function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <>
      <div className="bg-card rounded-2xl shadow-sm p-4 flex flex-col gap-2 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 hover:bg-secondary/40">
